...
void inicio::GenerateBotonera(){
    ...
    for(int i=0; i<tam; i++){
        ...
        for(int j=0; j<tam; j++){
            ...
            tam2.setHeight(this->buttons[i][j]->height()/(ui->spinBox->value()+20));
            tam2.setWidth(this->buttons[i][j]->width()/(ui->spinBox->value()+20));
            this->buttons[i][j]->setIconSize(tam2);
            if(state){
...
void inicio::handleButton(int i, int j, bool state, bool pl){
    if(state){
        ...
        if(this->BatallaNaval->getimpact()){
            ui->HP_ENEMIGO->setValue(ui->HP_ENEMIGO->value()-ui->HP_ENEMIGO->value()/(this->BatallaNaval->Listado(false).sizeShip()+1));
        }
        ...
        if(this->BatallaNaval->getimpact()){
            ui->HP_ALIADO->setValue(ui->HP_ALIADO->value()-ui->HP_ALIADO->value()/(this->BatallaNaval->Listado(true).sizeShip()+1));
        }
    }else{
        if(pl){
            ...
            this->BatallaNaval->battle(i, j, false, false);
            if(this->BatallaNaval->getimpact()){
                ui->HP_ALIADO->setValue(ui->HP_ALIADO->value()-ui->HP_ALIADO->value()/(this->BatallaNaval->Listado(true).sizeShip()+1));
            }
        }
...
void inicio::actualizarMapa(){
    ...
    for(int i=0; i<tam; i++){
        for(int j=0; j<tam; j++){
            ...
            case 'X':
                this->buttons[i][j]->setDisabled(true);
                this->buttons[i][j]->setStyleSheet("background-color:red; color:black; font-size : 10%; padding:1px; margin : 1px");
                this->buttons[i][j]->setText(QString::fromStdString("IMPACTO"));
                break;
            case 'O':

                this->buttons[i][j]->setDisabled(true);
                this->buttons[i][j]->setStyleSheet("background-color:blue; color:black;font-size : 10%; padding:1px; margin : 1px");
                this->buttons[i][j]->setText(QString::fromStdString("AGUA"));
                break;
            }
            switch(mapa2[i][j]){
            ...
            if(mapilab2[i][j] == 9){
                this->buttons[i][j]->setStyleSheet("background-color:#f80000; color:black; font-size: 10px; padding:1px; margin : 1px");
                this->buttons[i][j]->setText(QString::fromStdString("HUNDIDO"));
            }
        }
...
void inicio::on_Buton_ADD_Ship_released(){
    ...
    if(!band){
        QMessageBox::information(this, "Fallo", QString::fromStdString("Barco ya existente, o posicionamiento imposible"));
    }
}
...
void inicio::on_start_combat_clicked(){
    ...
    ui->widget_7->show();
    ui->LabelNombre->show();
    ui->LabelNombre->setText(QString::fromStdString("Almirante: "+this->BatallaNaval->Pl(true).getName()));
    ui->HP_ALIADO->setValue(100);
    ui->HP_ENEMIGO->setValue(100);
    ui->widget_11->show();
...
void inicio::RoutineStart(){
    QIcon icon("img/boton_Pause.jpg");
    ui->LabelNombre->hide();
    ui->play_stop->setIcon(icon);
    ui->widget_11->hide();
    ui->widget_10->show();
    ui->widget_6->show();
    ui->widget_8->show();
...
void inicio::on_ButonLoadGame_clicked(){
    ...
    QString filepath = QFileDialog::getOpenFileName(this, "Elija el archivo a cargar", "saveGame/", filtro);
    if(filepath.toStdString()!=""){
        QMessageBox::information(this, "EXITO", "partida Cargada");
...
void inicio::on_ExitButton_clicked(){
    if(this->BatallaNaval!=nullptr) EliminarGUI();
    exit(-1);
}


void inicio::on_play_stop_clicked(){
    if(this->audioOutput->isMuted()){
        QIcon icon("img/boton_Pause.jpg");
        ui->play_stop->setIcon(icon);
        this->audioOutput->setMuted(false);
    }else{
        QIcon icon("img/boton_Play.jpg");
        ui->play_stop->setIcon(icon);
