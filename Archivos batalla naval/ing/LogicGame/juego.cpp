...
void juego::Saveinf(std::string name, int tam, int cShips){
    NSave Jugador;
    std::fstream archi("juego.bin", std::ios::out|std::ios::trunc|std::ios::binary);
    std::fstream archi2("juego.txt", std::ios::out|std::ios::trunc);
    if(archi.is_open()){
        strcpy(Jugador.nombre, name.c_str());
...
void juego::generacionArchivo(){
    ...
    if(archi.is_open()){
        ...
        cont = 0;
        archi3<<"Movimientos Mquinas: "<<std::endl;
